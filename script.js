...
window.addEventListener('load', () => {
    ...
    // 2. INIT 3D EFFECT ON BOOT SCREEN
    init3DEffect();

    // 3. BOOT LOADER UI
    setTimeout(() => document.getElementById('boot-bar').style.width = "100%", 500);
    setTimeout(() => document.getElementById('start-btn').classList.remove('hidden'), 2500);
    ...
    document.getElementById('start-btn').addEventListener('click', () => {
        ...
        screen.style.transform = "scale(1.1)"; // Efek zoom out saat keluar
...
// --- NEW FEATURE: 3D PARALLAX EFFECT ---
function init3DEffect() {
    ...
    container.addEventListener('mousemove', (e) => {
        ...
        // Hitung rotasi berdasarkan posisi mouse dari tengah layar
        const yRotation = ((clientX / innerWidth) - 0.5) * 30; // Max 15deg rotasi Y
        const xRotation = ((clientY / innerHeight) - 0.5) * -30; // Max 15deg rotasi X (negatif agar natural)
...
// --- FITUR LAINNYA (SAMA SEPERTI SEBELUMNYA) ---
function detectOSForVoice() {
    ...
    else finalSpeech = "System Online. Welcome to Jasa Kimjong.";
}
