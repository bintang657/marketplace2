...
window.addEventListener('load', () => {
    ...
    // 3. ENTER SYSTEM (CLICK TRIGGER)
    document.getElementById('start-btn').addEventListener('click', () => {
        // Play SFX
        document.getElementById('bootSound').play().catch(e => console.log("Audio blocked:", e));
        
        // TRIGGER AI VOICE (JARVIS STYLE)
        speakAI("System Online. Welcome back, Administrator.");
...
// --- FUNGSI SUARA AI YANG DIPERBAIKI ---
function speakAI(text) {
    if ('speechSynthesis' in window) {
        ...
        // Coba cari suara Google US English (Suara Robot Terbaik)
        const voices = window.speechSynthesis.getVoices();
        const robotVoice = voices.find(v => v.name.includes('Google US English') || v.name.includes('Male'));
        
        if (robotVoice) utterance.voice = robotVoice;
        
        // Setting agar terdengar seperti AI
        utterance.lang = 'en-US';
        utterance.pitch = 0.8; // Agak ngebass
        utterance.rate = 0.9;  // Agak lambat & jelas
...
function initTypewriter() {
    ...
    if(isDeleting) {
        typeEl.innerText = currentWord.substring(0, charIndex--);
    } else {
        typeEl.innerText = currentWord.substring(0, charIndex++);
    }
...
document.querySelectorAll('a, button, .tilt-card').forEach(el => {
    el.addEventListener('mouseenter', () => { 
        ...
        document.getElementById('hoverSound').play().catch(e=>{}); 
    });
