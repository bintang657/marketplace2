...
// GLOBAL VARIABLES
let finalSpeech = "System Online. Welcome back, Administrator."; // Default Fallback

window.addEventListener('load', () => {
    // 1. DETECT OS IMMEDIATELY
    detectOSForVoice();
...
// --- FITUR UTAMA: DETEKSI OS UNTUK SUARA ---
function detectOSForVoice() {
    const ua = navigator.userAgent;

    if (/Android/i.test(ua)) {
        // JIKA PENGGUNA ANDROID
        finalSpeech = "Android System Detected. Access Granted.";
    } 
    else if (/iPhone|iPad|iPod/i.test(ua)) {
        // JIKA PENGGUNA IPHONE/IOS
        finalSpeech = "iOS Device Verified. Welcome to the Network.";
    } 
    else if (/Win/i.test(ua)) {
        // JIKA PENGGUNA WINDOWS (PC/LAPTOP)
        finalSpeech = "Windows System Detected. Security Level 0. Welcome back.";
    } 
    else {
        // JIKA LAINNYA (MAC/LINUX)
        finalSpeech = "System Online. Welcome back, Administrator.";
    }
    
    console.log("AI Voice Set To:", finalSpeech); // Cek di console browser
}
...
function speakAI(text) {
    if ('speechSynthesis' in window) {
        ...
        msg.text = text;
        msg.volume = 1; 
        msg.rate = 0.9; // Speed agak pelan biar dramatis
        msg.pitch = 0.8; // Suara agak ngebass (Robotik)
        msg.lang = 'en-US';

        // Cari suara Google US English (Paling bagus untuk robot)
...
// --- VISUAL & SYSTEM LOGIC (SAMA SEPERTI SEBELUMNYA) ---
