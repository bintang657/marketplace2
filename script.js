...
window.addEventListener('load', () => {
    ...
    // 3. ENTER SYSTEM (VOICE & PARTICLES)
    document.getElementById('start-btn').addEventListener('click', () => {
        document.getElementById('bootSound').play().catch(e=>{});
        
        // AI VOICE (Browser TTS)
        if ('speechSynthesis' in window) {
            const u = new SpeechSynthesisUtterance("System Online. Welcome back, Administrator.");
            u.pitch = 0.8; u.rate = 1; u.volume = 0.7;
...
function initWAWidget() { setTimeout(()=>{const b=document.getElementById('wa-bubble');b.classList.remove('opacity-0', 'translate-y-4'); document.getElementById('notifSound').play().catch(e=>{}); setTimeout(()=>b.classList.add('opacity-0'), 10000);}, 5000); }
